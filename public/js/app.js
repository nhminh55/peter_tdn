...
// Câu hỏi thuộc mảng theo q.part ('letter' / 'text' / 'passage'); không có part là Writing.
...
const sourceLabel = source => source.map(s => {
  if (s.gen) return t('src_gen');
...
// Câu Reading chọn đáp án (True/False, A/B/C/D) hay phải tự gõ (tìm từ, điền khuyết, câu tự viết).
const isChoice = q => q.type === 'reading_tf' || q.type === 'reading_mc';
const optionText = (q, letter) => (q.options && q.type === 'reading_mc' ? q.options[LETTERS.indexOf(letter)] : undefined);
...
function questionLabel(q) {
  ...
  if (q.prompt) return `${where}${q.num}. ${q.prompt}`;
  return `${where}(${q.num}) ${q.options ? q.options.join(' / ') : '______'}`;
}
const answerLabel = (q, a) => (q && isReading(modOf(q)) && isChoice(q) ? choiceLabel(q, a) : a);
...
function viewHome() {
  ...
  const readingLessons = READING_MODS.reduce((n, m) => n + MODS[m].lessons.length, 0);
  const readingQs = READING_MODS.reduce((n, m) => n + modQuestions(m).length, 0);
  page(`
    <section class="hero">
...
function viewReading() {
  const card = m => {
    ...
    return `
      <a class="big-choice part ${m}" href="${MODS[m].base}">
        <span class="bc-icon">${{ letter: '✉️', text: '📰', passage: '📚' }[m]}</span>
  ...
  page(`
    ...
    ${guest ? guestBanner() : ''}
    <section class="big-choices three">${READING_MODS.map(card).join('')}</section>`);
}
...
function passageTextHtml(raw, evidence, gaps) {
  ...
  const fill = str => esc(str).replace(/\((\d{1,2})\)_{3,}/g, (all, n) => {
    const g = gaps[n] || {};
...
function passageHtml(p, q, s, pending) {
  ...
  s.results.forEach(r => {
    ...
    if (!rq || rq.passage !== p.id || !inPassageBlank(rq)) return;
    const show = v => (isChoice(rq) ? optionText(rq, v) : v);
  ...
  return `
    <article class="passage ${p.part}${framed ? ' framed' : ''}">
      <div class="passage-head"><span class="passage-label">${label}</span><span class="q-src">${sourceLabel([p.source])}</span></div>
      ${p.intro ? `<p class="p-intro">${esc(p.intro)}</p>` : ''}
      ${p.part === 'passage' && !framed ? `<h3 class="p-title">${esc(p.title)}</h3>` : ''}
      ${p.box ? `<div class="word-box" aria-label="${t('word_box')}">${p.box.map(w => `<span>${esc(w)}</span>`).join('')}</div>` : ''}
      ${paras.map((para, i) => {
        if (para.startsWith('# ')) return `<h4 class="p-sub">${passageTextHtml(para.slice(2), evidence, gaps)}</h4>`;
        const cls = framed ? (i === 0 ? ' class="greet"' : i === last ? ' class="sign"' : '') : para.startsWith('- ') ? ' class="bullet"' : '';
...
function choicesHtml(q, pending, picked) {
  ...
  return `<div class="choices ${q.type === 'reading_tf' ? 'tf' : 'mc'}" role="radiogroup" aria-label="${t('choices_label')}">${values.map(v => {
    ...
    const text = q.options ? q.options[LETTERS.indexOf(v)] : v;
...
// Ô gõ đáp án: tìm từ / điền khuyết (một dòng), câu tự viết (nhiều dòng).
function typedInputHtml(q, pending) {
  const value = pending ? esc(pending.result.userAnswer) : '';
  ...
  const field = q.type === 'reading_open'
    ? `<textarea id="answer" rows="3" maxlength="300" placeholder="${t('ph_open')}" spellcheck="false" autocomplete="off" autocapitalize="off" ${ro}>${value}</textarea>`
    : `<input id="answer" class="word-input" type="text" maxlength="120" placeholder="${t(q.type === 'reading_word' ? 'ph_word' : 'ph_gap')}" spellcheck="false" autocomplete="off" autocapitalize="off" value="${value}" ${ro}>`;
  return opts + field;
}

const typedLabel = q => (q.type === 'reading_word' ? t('q_label_word') : q.type === 'reading_open' ? t('q_label_open')
  : q.prompt ? t('q_label_gap') : t('q_label_blank', { n: q.num }));
...
function viewReadingRun(m, s, q) {
  ...
  const prompt = q.prompt ? esc(q.prompt).replace(/_{3,}/, '<span class="gap-line"></span>')
    : t(choice ? 'blank_prompt' : 'gap_prompt', { n: q.num });
  const limit = q.maxWords ? `<span class="rq-limit">${t(q.maxWords === 1 ? 'limit_one' : 'limit_words', { n: q.maxWords })}</span>` : '';
  const keysHint = choice ? t(q.type === 'reading_tf' ? 'keys_tf' : 'keys_mc') : t(q.type === 'reading_open' ? 'keys_open' : 'enter_check');
  let picked = null;
      ...
      <div class="read-side">
        <section class="q-card" id="q-card">
          <div class="q-head">
            ...
            <span class="q-src">${t('q_num', { n: q.num })}${q.authored ? ` · ${t('authored')}` : ''}</span>
          </div>
          <div class="rq-prompt">${prompt}${limit}</div>
          ${choice ? choicesHtml(q, pending, picked) : typedInputHtml(q, pending)}
          <div class="q-meta"><span id="wc" class="wc"></span><span class="kbd-hint">${pending ? t('enter_next') : keysHint}</span></div>
          <div id="hint" class="hint" hidden></div>
          <div id="submit-error" class="submit-error" hidden></div>
          <div class="q-actions">
            ${pending ? `
  ...
  const wc = $('#wc');
  const updateWc = () => {
    if (!field || !q.maxWords) return;
    const n = countWords(field.value);
    wc.textContent = t('words_of', { n, max: q.maxWords });
    wc.classList.toggle('over', n > q.maxWords);
  };
  updateWc();
  ...
  if (pending) {
    ...
    setKeys(e => { if (e.key === 'Enter' && (!typing(e) || e.target === field)) { e.preventDefault(); nextQuestion(m); } });
    next.focus({ preventScroll: true });
    $('#feedback').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    return;
  }
  ...
  if (choice) {
    ...
    const keyMap = q.type === 'reading_tf' ? { t: 'True', f: 'False', 1: 'True', 2: 'False' }
      : Object.fromEntries(LETTERS.slice(0, q.options.length).flatMap((l, i) => [[l.toLowerCase(), l], [String(i + 1), l]]));
    setKeys(e => {
      if (typing(e)) return;
    ...
    field.addEventListener('input', updateWc);
    field.addEventListener('keydown', e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); check(); } });
  }
  btnCheck.addEventListener('click', check);
  $('#btn-hint').addEventListener('click', () => {
    const h = $('#hint');
  ...
  async function check() {
    if (busy) return;
    const answer = choice ? picked : field.value.trim();
    if (!answer) {
      const box = choice ? $('.choices') : field;
      box.classList.add('shake');
      setTimeout(() => box.classList.remove('shake'), 400);
      if (!choice) field.focus();
      return;
    }
    busy = true;
    btnCheck.disabled = true;
    ...
    $('#submit-error').hidden = true;
    try {
      applyResult(s, await submitAnswer(s, q, answer));
    } catch (err) {
      console.error(err);
      busy = false;
      btnCheck.disabled = false;
...
function readingFeedbackHtml(r, q) {
  const choice = isChoice(q);
  const issues = choice ? [] : (r.issues || []).map(i => t('rissue_' + i.code, { n: i.n, w: esc(r.closest || '') }));
  const banner = r.correct
    ...
    : choice
      ...
      : `<div class="banner bad"><span class="b-icon">🤔</span><div>${t(q.type === 'reading_open' ? 'fb_bad_open' : 'fb_bad_typed')}${issues.length ? `<ul class="banner-list">${issues.map(x => `<li>${x}</li>`).join('')}</ul>` : ''}</div></div>`;
  ...
  } else if (q.type === 'reading_open') {
    const ideas = (r.keyIdeas || []).map((idea, i) => {
      const ok = r.ideas && r.ideas[i];
      return `<li class="${ok ? 'ok' : 'miss'}"><span class="idea-mark">${ok ? '✓' : '✗'}</span>${esc(idea[lang()] || idea.vi)}</li>`;
    }).join('');
    compare = `
      <div class="cmp-row"><span class="cmp-label">${t('cmp_wrote')}</span><div class="cmp-text ${r.correct ? 'ok' : 'bad'}">${esc(r.userAnswer)}</div></div>
      ${ideas ? `<div class="cmp-row"><span class="cmp-label">${t('cmp_ideas')}</span><ul class="idea-list">${ideas}</ul></div>` : ''}
      <div class="cmp-row"><span class="cmp-label">${t('cmp_sample')}</span><div class="cmp-text book">${esc(r.answer)}</div></div>`;
  } else {
    const same = window.ReadingGrader.normalize(r.userAnswer) === window.ReadingGrader.normalize(r.answer);
    const others = (r.answers || []).filter(a => a !== r.answer && window.ReadingGrader.normalize(a) !== window.ReadingGrader.normalize(r.userAnswer));
    compare = `
      <div class="cmp-row"><span class="cmp-label">${t('cmp_wrote')}</span><div class="cmp-text ${r.correct ? 'ok' : 'bad'}">${esc(r.userAnswer)}</div></div>
      ${same ? '' : `<div class="cmp-row"><span class="cmp-label">${t('cmp_right')}</span><div class="cmp-text ok">${esc(r.answer)}</div></div>`}
      ${others.length ? `<div class="cmp-row"><span class="cmp-label">${t('cmp_also')}</span><div class="cmp-text">${others.map(esc).join(' · ')}</div></div>` : ''}`;
  }
  return `
